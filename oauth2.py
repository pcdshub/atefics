...
def SetupOptionParser():
  ...
  parser.add_option('--smtp_authentication',
                    action='store_true',
                    dest='smtp_authentication',
...
def SmtpAuthentication(auth_string):
...
def main(argv):
  ...
  if options.refresh_token:
    ...
    return RefreshToken(options.client_id, options.client_secret,
    ...
    TestImapAuthentication(
        GenerateOAuth2String(options.user, options.access_token,
                             base64_encode=False))
  elif options.smtp_authentication:
    RequireOptions(options, 'user', 'access_token')
